		...
		<%-- 	${val }
			${music.musicVideo }입니다  --%>
			   <c:forEach items="${list}" var="MusicVO">
     				<li song="로꼬, GRAY (그레이) - GOOD (Feat. ELO).mp3" musicLyrics="${MusicVO.musicLyrics}" cover="${MusicVO.musicVideo }/0.jpg" artist="${MusicVO.artistName }" id="val">${MusicVO.artistName } - ${MusicVO.musicSubject }</li>
   				</c:forEach>
		</ul>
