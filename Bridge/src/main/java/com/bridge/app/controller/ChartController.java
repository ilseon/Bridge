...
public class ChartController {
   ...
   public String chart(String genre, Model view, HttpServletRequest req) throws Exception{
      ...
      req.setAttribute("realpath", req.getSession().getServletContext().getRealPath("/resources/image/upload/album/")+"\\");
   ...
   public String myalbum_modal(@RequestParam("musicnumber") int musicnumber, Model view, HttpServletRequest req) throws Exception{
      ...
      if(list.size()>0){
         ...
         logger.info("/chart/modal/myalbum_already");
   ...
   public String myalbum_modal_sev(@RequestParam("playlistAll") List<Integer> playlistAll, Model view, HttpServletRequest req) throws Exception{
      ...
      map.put("usernumber", usernumber);
      map.put("playListAll", playlistAll);
      logger.info(playlistAll.size()+"myalbum_modal_sev"+usernumber);
      
      List<PlaylistVO> list = playlist.search_myalbum(map);
      if(list.size()==playlistAll.size()){//선택한 곡들이 모두 내 앨범에 있는 곡들일 때
         logger.info("선택한곡들이모두내앨범에있는곡들일때");
         return "chart/modal/myalbum_already";
      }else if(list.size()==0){//선택한 곡들이 모두 새롭게 추가하는 곡일 때
         logger.info("선택한 곡들이 모두 새롭게 추가하는 곡일 때");
         map.put("playListAll", playlistAll);
   ...
   public String real_path(HttpServletRequest req){
      return req.getSession().getServletContext().getRealPath("/resources/image/upload/music/")+"\\";
   }
